...
export default async function NewSessionPage({ params }: PageProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h1 className="text-2xl font-black text-brand-navy-900 dark:text-foreground">New session</h1>
        <p className="mt-1 text-sm text-muted-foreground">{`Add a session to ${season.title}.`}</p>
      </div>
      <Card>
