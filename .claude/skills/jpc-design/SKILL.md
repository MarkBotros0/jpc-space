...
> The init script must be a plain `<script dangerouslySetInnerHTML={{ __html: themeInitScript }} />` in `app/layout.tsx` — **not** `next/script`'s `<Script>` with text children (React 19 warns "Encountered a script tag…") and **not** `<script>{themeInitScript}</script>` (same warning). `dangerouslySetInnerHTML` has no React children, so it renders raw and runs pre-hydration without warning.
...
- **Bottom nav** (`<md`) — a **floating frosted pill**, not a full-width edge bar: `fixed inset-x-4 bottom-[max(1rem,env(safe-area-inset-bottom))] mx-auto max-w-md h-16 rounded-full`, translucent navy glass (`bg-sidebar/75 backdrop-blur-xl backdrop-saturate-150`, `ring-white/10`, `shadow-lift`) that stays dark in both themes (do **not** use `jpc-glass` here — it'd go light in light mode). 5 items max from `tabs` in `src/lib/navigation.ts`. Each tab is **icon over label** with light icons (`text-white/55`, active `text-white`) and a sliding circular active highlight (`layoutId="nav-tab-active"`, `bg-white/15`). **Home is the centre (3rd) tab** in every role and renders as a distinct **teal featured button** (`bg-brand-teal-500`, navy icon) via `NavLink`'s `featured` prop — keep Home centred in each role's `tabs` array. The **More** page (`more-menu.tsx`) auto-derives its list as `sidebar − tabs`: any item that's in `sidebar` but not in `tabs` shows up under More, so editing `tabs` silently changes the More menu.
